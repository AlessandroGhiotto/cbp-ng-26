...
Building upon this comparative study, we propose a variant of the state-of-the-art \textbf{TAGE}~\cite{seznec2006case} (Tagged Geometric History Length) architecture. TAGE uses multiple tagged predictor tables indexed with geometrically increasing history lengths to capture both short-term path correlations and long-term global context. By analyzing our results we have noticed that the TAGE predictor has a very low MPKI but an high EPI, so in our proposal we decided to decrease the energy consumed by skipping the TAGE lookup for biased branches or saturated branches.
...
\textbf{Block vs. Scalar Predictors:} Block-level predictors (suffix \texttt{L}) fetch and predict multiple instructions in a single cycle by reusing the active SRAM row, leading to an IPC range of 3.0--4.7, compared to only 0.49--0.98 for scalar models. More importantly, this throughput dominance translates directly into energy efficiency: block predictors achieve VFS speedup factors of 3.0x--5.64x over their scalar counterparts. Since predicting SRAM lines is efficient, the Energy Per Instruction (EPI) remains competitive.

\textbf{Predictor Groups and Behaviors:} The global-indexed models (\texttt{gap}, \texttt{gag}, and \texttt{gshare}) behave very similarly, as they use variations of global history XORed or concatenated with the PC address. 
% yielding clustered VFS and MPKI scores (e.g., VFS of 0.6586, 0.6525, 0.6472 respectively at 16 KB block). 
When comparing TAGE variants, \texttt{tageL} achieves a higher IPC (4.7048 vs. 4.2656) due to simpler update rules, whereas \texttt{tage\_uL} achieves a lower MPKI (4.19 vs. 5.42) by utilizing usefulness (\texttt{u}) bits to track and deallocate useless entries, highlighting the accuracy-throughput trade-off.

\textbf{The 16 KB vs. 8 KB Budget Trade-off:} Doubling the budget from 8 KB to 16 KB reduces the branch misprediction rate. However, under HARCOM's physical timing and energy model, a larger budget is not guaranteed to improve the VFS score. Higher capacity increases SRAM access latency and dynamic/static energy (e.g., \texttt{tageL} EPI increases from 939.6 fJ at 8 KB to 1145.5 fJ at 16 KB). As a result, the average VFS score for several predictors actually degrades when moving from 8 KB to 16 KB (e.g., \texttt{tageL} VFS drops from 0.7101 to 0.6988).

\subsection{Our Proposal}
Finally here we have the our proposed TAGE variants.  

\texttt{\textbf{tage\_bimodeL}:} we try to get better first level predictions by using a Bimode predictor instead of a simple Bimodal PHT. 

\textbf{\texttt{tage\_biasL}:} we use a bias table (proposed in the BiMode++ paper) as a "first filter", which allows to use less energy by avoiding completely the TAGE lookup for this biased branches. It decreased the EPI, but it also give more mispredictions, decreasing the IPC.

\textbf{\texttt{tage\_simple\_satL}:} the bad results of the bias table lead us to a more flexible handling of biased branches. In this variant we skip the TAGE lookup when the base predictor counters are saturated.
The experimental results shows that this version is competitive with the Tage predictor, by playing with the trade-off between MPKI and EPI. This idea was given by the fact of trying to use "the right tool for the right need", if we could filter branches by classes and have many ad-hoc small predictors for each of them, we could spare much energy instead of going with the full TAGE predictor.
