...
Doubling the budget from 8 KB to 16 KB reduces the branch misprediction rate. However, under HARCOM's physical timing and energy model, a larger budget is not guaranteed to improve the VFS score. Higher capacity increases SRAM access latency and dynamic/static energy (e.g., \texttt{tageL} EPI increases from 939 fJ at 8 KB to 1145 fJ at 16 KB). As a result, the average VFS score for several predictors actually degrades when moving from 8 KB to 16 KB (e.g., \texttt{tageL} VFS drops from 0.7101 to 0.6988).

\subsection{Our Proposal}
Building upon the trade-off analysis of block-level TAGE predictors, we propose three distinct TAGE variants designed to optimize energy efficiency and prediction throughput:

\vspace{0.2cm}
\textbf{TAGE bimode.} \\
This variant replaces the simple bimodal base predictor ($t_0$) with a choice-directed Bi-Mode predictor~\cite{lee1997bimode}. However, under strict memory budgets (8~KB and 16~KB), the choice table, taken PHT, and not-taken PHT consume a significant fraction of the storage. This severely limits the capacity of the TAGE tagged tables, forcing them to use short 8-bit tags and fewer rows, causing the average MPKI to degrade to 9.74 (compared to 5.42 for \texttt{tageL}). Additionally it consumes also more energy than a bimodal basae predictor.
...
\begin{itemize}
    \item \textbf{Throughput:} It achieves the highest average IPC among all evaluated predictors (4.8512 at 16~KB and 4.8579 at 8~KB). Bypassing the Stage 2 tagged RAM lookups for saturated branches eliminates their read latency, allowing Stage 2 overrides to resolve faster.
    \item \textbf{Energy:} It significantly reduces dynamic energy, achieving a 14.6\% reduction in EPI at 16~KB and a 21.3\% reduction at 8~KB compared to \texttt{tageL}.
...
This section presents the performances of all the predictors (both scalar and block based) averaged over the 12 traces, with VFS, MPKI, IPC and EPI. Then we have the table highlighting the speedup in VFS score by comparing the scalar vs the block version.
