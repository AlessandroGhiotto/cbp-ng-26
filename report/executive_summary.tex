...
Block-level predictors (suffix \texttt{L}) fetch and predict multiple instructions in a single cycle (up to 16 instructions, as $LI=16$) by reusing the active SRAM row, leading to a massive IPC boost (ranging from 3.0 to 4.85) compared to scalar models (which are strictly limited to an IPC range of 0.49--0.98). More importantly, this throughput dominance translates directly into energy efficiency. For example, the block-level \texttt{gshareL} (16~KB) achieves an EPI of 145.5~fJ, which is nearly half the EPI of its scalar counterpart \texttt{gshare} (263.2~fJ), despite executing at a 7x higher IPC. This occurs because scalar predictors must perform a separate SRAM read access for every single instruction, whereas block predictors amortize a single wide SRAM row read over the entire fetch block, fetching subsequent predictions from low-power registers (using \texttt{reuse\_predict1/2}). This reduction in active SRAM accesses results in a 3.0x--5.64x speedup in VFS score for block-level models over their scalar counterparts.
