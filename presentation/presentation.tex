...
\begin{frame}{Introduction \& Objectives}
    \begin{block}{Background \& Problem}
        \begin{itemize}
            \item \textbf{Speculative Execution:} Critical for modern out-of-order superscalar performance.
            \item \textbf{Misprediction Cost:} Pipeline flushes cause severe IPC loss and energy waste.
        \end{itemize}
    \end{block}
    \begin{block}{Project Goals}
        \begin{itemize}
            \item \textbf{Exploration:} Benchmark classical global and local history predictors under strict \textbf{8~KB} and \textbf{16~KB} budgets.
...
\begin{frame}{Block-Level Prediction: Amortizing Lookup Overhead}
    \begin{columns}[T]
        \column{0.50\textwidth}
        \begin{block}{SRAM Access Amortization}
            \begin{itemize}
                \item \textbf{Scalar Bottleneck:} SRAM lookup per instruction limits bandwidth ($\text{IPC} < 1.0$).
                \item \textbf{Block Prediction (L):} Predicts up to $LI = 16$ instructions in a single wide SRAM row access.
                \item \textbf{Register Reuse:} Cache active row bits to bypass subsequent lookups.
            \end{itemize}
        \end{block}
        ...
        \begin{block}{Key Benefits}
            \begin{itemize}
                ...
                \item \textbf{Energy reduction:} Amortized lookups cut EPI:
                \begin{itemize}
                    \item Scalar \texttt{gshare}: 263.2 fJ
                    \item Block \texttt{gshareL}: \textbf{145.5 fJ} ($\approx 45\%$ savings) \hyperlink{block_16kb}{\beamerbutton{16KB Table}}
                \end{itemize}
    ...
    \begin{columns}[T]
        ...
        \begin{block}{Accuracy \& Energy Challenges}
            \begin{itemize}
                ...
                \item \textbf{3 Tagged Tables ($T_1, T_2, T_3$):} Hist. of 4, 16, 64 bits.
                \item \textbf{Replacement:} Refined usefulness ($u$) tracking.
                \item \textbf{Accuracy:} Peak at \textbf{4.19 MPKI} (\texttt{tage\_uL}) \hyperlink{block_16kb}{\beamerbutton{16KB Table}}.
                \item \textbf{Parallel Lookup:} Concurrent reads to $T_1, T_2, T_3$ in Stage 2.
                \item \textbf{High EPI:} \texttt{tageL} at \textbf{1145.5 fJ} ($\approx 8\times$ vs. \texttt{gshareL}) \hyperlink{block_16kb}{\beamerbutton{16KB Table}}.
            \end{itemize}
        \end{block}
        ...
        \IfFileExists{images/tage_architecture.png}{%
            \begin{center}
                \includegraphics[width=1.04\textwidth, height=4.4cm, keepaspectratio]{images/tage_architecture.png}
            \end{center}
        }{%
...
\section{Proposed Methodologies: TAGE Bimode}
\begin{frame}{Proposed Methodologies: TAGE Bimode}
    \small
    \begin{columns}[T]
        \column{0.52\textwidth}
        \begin{block}{Concept \& Motivation}
            \begin{itemize}
                \item \textbf{Choice-Directed Base Table:} Splits predictions into separate taken/not-taken tables to minimize destructive aliasing.
        ...
        \begin{block}{Resource Constraints \& Aliasing}
            \begin{itemize}
                ...
                \item \textbf{Aliasing Penalty:} Forces short tags (8-bit) $\rightarrow$ severe aliasing (\textbf{9.74 MPKI}) and high power (\textbf{1587.4 fJ EPI}) \hyperlink{block_16kb}{\beamerbutton{16KB Table}}.
            \end{itemize}
        \end{block}
        ...
        \IfFileExists{images/bimode_architecture.png}{%
            \begin{center}
                \includegraphics[width=\textwidth, height=4.8cm, keepaspectratio]{images/bimode_architecture.png}
            \end{center}
        }{%
...
\section{Proposed Methodologies: TAGE Bias}
\begin{frame}{Proposed Methodologies: TAGE Bias}
    ...
    \begin{columns}[T]
        ...
        \begin{block}{Concept \& Motivation}
            \begin{itemize}
                \item \textbf{Bias Filter Table:} Employs a dedicated table to identify strongly biased branches early.
                \item \textbf{Power Savings Path:} Directly resolves predictions from registers, bypassing expensive TAGE SRAM lookups.
            \end{itemize}
        \end{block}
        \vspace{0.05cm}
        \begin{block}{Critical Path \& Latency Penalty}
            \begin{itemize}
                \item \textbf{Serialization:} Sequential checking of the bias table in Stage 1 extends the critical path ($>300$ ps).
                \item \textbf{Throughput Drop:} Halves the operating frequency $\rightarrow$ IPC drops to \textbf{2.83} \hyperlink{block_16kb}{\beamerbutton{16KB Table}}.
            \end{itemize}
        \end{block}
        ...
        \IfFileExists{images/bias_architecture.png}{%
            \begin{center}
                \includegraphics[width=1.06\textwidth, height=5.2cm, keepaspectratio]{images/bias_architecture.png}
            \end{center}
        }{%
...
\begin{frame}{Our Final Proposal: TAGE sat (Saturation Bypass)}
    ...
    \begin{columns}[T]
        ...
        \begin{block}{Core Mechanism}
            \begin{itemize}
                \item \textbf{Bypass Condition:} Saturated bimodal base counters (0 or max) $\rightarrow$ strongly biased.
                \item \textbf{Dynamic Bypass:} Bypasses tagged SRAM lookups ($t_1, t_2, t_3$) using HARCOM's \texttt{execute\_if}.
            \end{itemize}
        \end{block}
        \vspace{0.05cm}
        \begin{block}{Throughput \& Power Optimization}
            \begin{itemize}
                \item \textbf{Throughput:} Bypasses Stage 2 SRAM latency $\rightarrow$ \textbf{4.85 IPC} \hyperlink{block_16kb}{\beamerbutton{16KB Table}}.
                \item \textbf{Power:} Disables tagged table SRAM reads $\rightarrow$ cuts EPI by \textbf{14.6\% -- 21.3\%} \hyperlink{block_16kb}{\beamerbutton{16KB Table}}.
            \end{itemize}
        \end{block}
        ...
        \IfFileExists{images/sat_architecture.png}{%
            \begin{center}
                \includegraphics[width=1.04\textwidth, height=4.8cm, keepaspectratio]{images/sat_architecture.png}
            \end{center}
        }{%
...
\begin{frame}[label=main_results]{Experimental Evaluation (16 KB Budget)}
    \begin{columns}[T]
        \column{0.44\textwidth}
        \begin{block}{Key Results \& Insights}
            \begin{itemize}
                \item \textbf{Peak IPC:} \texttt{tage\_satL} achieves the highest throughput (\textbf{4.85} vs. 4.70 reference).
                \item \textbf{Energy Savings:} \texttt{tage\_satL} cuts EPI by \textbf{14.6\%} (16~KB) and \textbf{21.3\%} (8~KB) vs. reference \hyperlink{block_8kb}{\beamerbutton{8KB Table}}.
                \item \textbf{VFS Trade-off:} \texttt{tageL} reference leads VFS (0.6988) due to better accuracy (5.42 MPKI).
        ...
        \begin{table}[H]
            ...
            \begin{tabular}{lcccc}
                ...
                \midrule
                \textbf{\texttt{tageL} (Ref)} & \textbf{0.6988} & 5.42 & 4.7048 & 1145.5 \\
...
\begin{frame}{Conclusions \& Future Work}
    \begin{block}{Conclusions}
        \begin{itemize}
            \item \textbf{Block-Level Advantage:} Crucial for out-of-order cores (up to $7\times$ IPC boost, $2\times$ EPI cut).
            \item \textbf{TAGE Saturation Bypass:} Achieves peak throughput (\textbf{4.85 IPC}) and cuts EPI by 14.6\%--21.3\%.
            \item \textbf{Complexity Penalty:} Complex add-ons degrade timing and storage efficiency.
        \end{itemize}
    \end{block}
    \begin{block}{Future Directions}
        \begin{itemize}
            \item \textbf{Predictor Hybridization:} Combine smaller, specialized predictors.
            \item \textbf{Dynamic Steering:} Route branches to the simplest accurate predictor to save energy.
        \end{itemize}
    \end{block}
\end{frame}
