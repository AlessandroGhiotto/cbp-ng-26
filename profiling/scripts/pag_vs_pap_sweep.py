...
def main():
    ...
    if not args.skip_sim:
        ...
        for B in [6, 8, 10, 12]:
            # PAG (PC_B2 = 0)
            configs.append(
                {"type": "pag", "expr": f"pag<{B},8,2>", "B": B, "PC_B2": 0, "BHR_B": B}
            )
            # PAP (PC_B2 = 2, 4, 6, ...)
        ...
        for cfg in configs:
            expr = cfg["expr"]
            print(
                f"Running: {expr} (PHT bits: {cfg['B']}, PC_B2: {cfg['PC_B2']}, Local History: {cfg['BHR_B']})"
            )
            try:
    ...
    fig.suptitle(
        "PAG vs PAP: Local History Indexing Space Sweep",
        fontsize=18,
        fontweight="bold",
        y=0.98,
    )
    ...
    ax2.set_title(
        "Performance (IPC) vs PHT Index Size", fontsize=13, fontweight="bold", pad=15
    )
    ...
    if subset_b:
        ...
        ax4.set_title(
            f"Optimal Split for Fixed Budget (B={fixed_b} bits)",
            fontsize=13,
            fontweight="bold",
            pad=15,
        )
