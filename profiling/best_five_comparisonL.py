...
def main():
    ...
    required = ["gagL<6,2,4>", "gapL<2,4,2,4>", "pagL<6,8,2,4>", "papL<2,8,6,2,4>", "bimodeL<8,10,8,2,4>"]
    missing = [r for r in required if r not in results]
    if missing:
        print(f"Warning: Missing sweep data for {missing}. Using hardcoded fallbacks.")
        fallback = {
            "gagL<6,2,4>": {"name": "gagL", "expr": "gagL<6,2,4>", "ipc": 5.0664, "mpi": 0.8602, "epi": 87.0, "vfs": 0.7765},
            "gapL<2,4,2,4>": {"name": "gapL", "expr": "gapL<2,4,2,4>", "ipc": 5.0864, "mpi": 0.7844, "epi": 87.0, "vfs": 0.7937},
            "pagL<6,8,2,4>": {"name": "pagL", "expr": "pagL<6,8,2,4>", "ipc": 2.4312, "mpi": 1.0664, "epi": 156.0, "vfs": 0.4781},
            "papL<2,8,6,2,4>": {"name": "papL", "expr": "papL<2,8,6,2,4>", "ipc": 2.5489, "mpi": 0.7894, "epi": 151.0, "vfs": 0.5197},
            "bimodeL<8,10,8,2,4>": {"name": "bimodeL", "expr": "bimodeL<8,10,8,2,4>", "ipc": 4.7450, "mpi": 0.6882, "epi": 267.0, "vfs": 0.7828}
        }
        for m in missing:
    ...
    comparison_list = [
        ...
        # Our Best Predictors
        {**results["gagL<6,2,4>"], "is_baseline": False, "name": f"gagL\n{results['gagL<6,2,4>']['expr']}"},
        {**results["gapL<2,4,2,4>"], "is_baseline": False, "name": f"gapL\n{results['gapL<2,4,2,4>']['expr']}"},
        {**results["pagL<6,8,2,4>"], "is_baseline": False, "name": f"pagL\n{results['pagL<6,8,2,4>']['expr']}"},
        {**results["papL<2,8,6,2,4>"], "is_baseline": False, "name": f"papL\n{results['papL<2,8,6,2,4>']['expr']}"},
        {**results["bimodeL<8,10,8,2,4>"], "is_baseline": False, "name": f"bimodeL\n{results['bimodeL<8,10,8,2,4>']['expr']}"}
    ]
    ...
    fig.suptitle("Best-of-Class Superscalar (Block) Predictors Comparison", fontsize=18, fontweight='bold', y=0.98)
