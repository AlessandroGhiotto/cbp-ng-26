...
def main():
    ...
    if not args.skip_sim:
        ...
        for g in log_ght_vals:
            for h in hist_bits_vals:
                expr = f"lxor<{g},{h}>"
                configs.append({"expr": expr, "log_ght": g, "hist_bits": h})

        print(f"Sweeping {len(configs)} LXOR configurations...")
        for cfg in configs:
            expr = cfg["expr"]
            print(
                f"Running: {expr} (LHT Size: {cfg['log_ght']} bits, History Width: {cfg['hist_bits']} bits)"
            )
            try:
                metrics = run_predictor(
                    expr, Path(args.trace).resolve(), args.warmup, args.measure
                )
    ...
    fig.suptitle(
        "LXOR Predictor: Design Space Sweep", fontsize=18, fontweight="bold", y=0.98
    )
    ...
    ax2.set_title(
        "Performance (IPC) vs History Width", fontsize=13, fontweight="bold", pad=15
    )
    ...
    ax4.set_title(
        "Top 5 LXOR Configurations (VFS Score)", fontsize=13, fontweight="bold", pad=15
    )
    ...
    for i, r in enumerate(results[:5]):
        print(
            f"{i+1}. {r['expr']}: VFS={r['vfs']:.4f}, MPI={r['mpki']/10.0:.3f}%, IPC={r['ipc']:.3f}, EPI={r['epi']:.1f} fJ"
        )
