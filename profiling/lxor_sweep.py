#!/usr/bin/env python3
"""
LXOR Parameter Space Explorer.
Finds the VFS-optimal LXOR configuration by sweeping LOG_GHT and HIST_BITS.
"""
...
def main():
    parser = argparse.ArgumentParser(description="Sweep LXOR Parameters")
    ...
    csv_path = out_dir / "lxor_sweep_results.csv"

    results = []

    if not args.skip_sim:
        # Define sweep space
        log_ght_vals = [6, 8, 10, 12]
        hist_bits_vals = [4, 5, 6, 7]

        configs = []
        for g in log_ght_vals:
            for h in hist_bits_vals:
                expr = f"lxor<{g},{h}>"
                configs.append({
                    "expr": expr,
                    "log_ght": g,
        ...
        print(f"Sweeping {len(configs)} LXOR configurations...")
        for cfg in configs:
            expr = cfg["expr"]
            print(f"Running: {expr} (LHT Size: {cfg['log_ght']} bits, History Width: {cfg['hist_bits']} bits)")
            try:
        ...
        with open(csv_path, "w", newline="") as f:
            fieldnames = ["log_ght", "hist_bits", "expr", "ipc", "cpi", "epi", "mpki", "vfs"]
        ...
        with open(csv_path, "r") as f:
            ...
            for row in reader:
                results.append({
                    "log_ght": int(row["log_ght"]),
                    "hist_bits": int(row["hist_bits"]),
                    "expr": row["expr"],
                    "ipc": float(row["ipc"]),
                    "cpi": float(row["cpi"]),
                    "epi": float(row["epi"]),
    ...
    # Generate 2x2 design space plots for LXOR
    print("\nGenerating LXOR sweep plots...")
    fig, axs = plt.subplots(2, 2, figsize=(16, 14))
    fig.suptitle("LXOR Predictor: Design Space Sweep", fontsize=18, fontweight='bold', y=0.98)
    ax1, ax2, ax3, ax4 = axs.flatten()

    log_ght_plot = [6, 8, 10, 12]
    colors = ['#e6194B', '#3cb44b', '#4363d8', '#f58231']
    
    # 1. MPI vs HIST_BITS
    for idx, g in enumerate(log_ght_plot):
        subset = [r for r in results if r["log_ght"] == g]
        subset = sorted(subset, key=lambda x: x["hist_bits"])
        if subset:
            h_vals = [r["hist_bits"] for r in subset]
            mpi_vals = [r["mpki"] / 10.0 for r in subset]
            ax1.plot(h_vals, mpi_vals, marker='o', linewidth=2.5, color=colors[idx], label=f"LHT Size = 2^{g}")
            
    ax1.set_title("Accuracy vs History Width", fontsize=13, fontweight='bold', pad=15)
    ax1.set_xlabel("History Width (HIST_BITS)", fontsize=11, fontweight='bold')
    ...
    for idx, g in enumerate(log_ght_plot):
        ...
        if subset:
            ...
            ax2.plot(h_vals, ipc_vals, marker='o', linewidth=2.5, color=colors[idx], label=f"LHT Size = 2^{g}")

    ax2.set_title("Performance (IPC) vs History Width", fontsize=13, fontweight='bold', pad=15)
    ax2.set_xlabel("History Width (HIST_BITS)", fontsize=11, fontweight='bold')
    ...
    for idx, g in enumerate(log_ght_plot):
        ...
        if subset:
            ...
            ax3.plot(h_vals, epi_vals, marker='o', linewidth=2.5, color=colors[idx], label=f"LHT Size = 2^{g}")

    ax3.set_title("Energy vs History Width", fontsize=13, fontweight='bold', pad=15)
    ax3.set_xlabel("History Width (HIST_BITS)", fontsize=11, fontweight='bold')
    ...
    # 4. Bar plot of Top 5 LXOR Configs by VFS
    top_5 = results[:5]
    bar_labels = [f"G={r['log_ght']}\nH={r['hist_bits']}" for r in top_5]
    bar_vfs = [r["vfs"] for r in top_5]
    
    bar_colors = ['#50E3C2', '#1abc9c', '#2ecc71', '#3498db', '#9b59b6']
    bars = ax4.bar(bar_labels, bar_vfs, color=bar_colors, edgecolor='#333333', linewidth=0.5, alpha=0.85, width=0.5)
    
    ax4.set_title("Top 5 LXOR Configurations (VFS Score)", fontsize=13, fontweight='bold', pad=15)
    ...
    plot_path = out_dir / "lxor_exploration.png"
    plt.savefig(plot_path, dpi=150)
    print(f"Saved exploration plot: {plot_path}")
    plt.close()

    # Print top 5
    print("\nTop 5 LXOR Configurations:")
