...
def main():
    ...
    if not args.skip_sim:
        ...
        for B in [6, 8, 10, 12]:
            # GAG (PC_B = 0)
            configs.append({"type": "gag", "expr": f"gag<{B},2>", "B": B, "PC_B": 0, "BHR_B": B})
            # GAP (PC_B = 2, 4, 6, ...)
            for pc in range(2, B, 2):
                bhr = B - pc
                configs.append({"type": "gap", "expr": f"gap<{bhr},{pc},2>", "B": B, "PC_B": pc, "BHR_B": bhr})
    ...
    fig.suptitle("GAG vs GAP: Architectural Indexing Space Sweep", fontsize=18, fontweight='bold', y=0.98)
