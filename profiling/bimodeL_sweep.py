#!/usr/bin/env python3
"""
Bi-Mode_L Parameter Space Explorer.
Finds the VFS-optimal bimodeL configuration by sweeping CHOICE_B, PHT_B, and BHR_B.
...
def main():
    parser = argparse.ArgumentParser(description="Sweep bimodeL Parameters")
    ...
    csv_path = out_dir / "bimodeL_sweep_results.csv"
    ...
    if not args.skip_sim:
        ...
        for c in choice_bits:
            for p in pht_bits:
                for b in bhr_bits:
                    expr = f"bimodeL<{c},{b},{p},2,4>"
                    configs.append(
                        {"expr": expr, "choice_b": c, "bhr_b": b, "pht_b": p}
                    )

        print(f"Sweeping {len(configs)} bimodeL configurations...")
        for cfg in configs:
            expr = cfg["expr"]
            print(
                f"Running: {expr} (Choice: {cfg['choice_b']}, BHR: {cfg['bhr_b']}, PHT: {cfg['pht_b']})"
            )
            try:
    ...
    # Generate 2x2 design space plots for bimodeL
    print("\nGenerating bimodeL sweep plots...")
    fig, axs = plt.subplots(2, 2, figsize=(16, 14))
    fig.suptitle(
        "bimodeL Predictor: Superscalar Design Space Sweep",
        fontsize=18,
        fontweight="bold",
        y=0.98,
    )
    ...
    for idx, c in enumerate(choices_to_plot):
        ...
        if subset:
            ...
            ax1.plot(
                ...
                linewidth=2.5,
                color=colors[idx],
                label=f"Choice Bits = {c}",
            )

    ax1.set_title(
        "Accuracy vs PHT Index Size (BHR_B=12)", fontsize=13, fontweight="bold", pad=15
    )
    ...
    ax2.set_title(
        "Performance (IPC) vs PHT Index Size (BHR_B=12)",
        fontsize=13,
        fontweight="bold",
        pad=15,
    )
    ...
    ax3.set_title(
        "Energy vs PHT Index Size (BHR_B=12)", fontsize=13, fontweight="bold", pad=15
    )
    ...
    # 4. Bar plot of Top 5 Configs by VFS
    top_5 = results[:5]
    ...
    ax4.set_title(
        "Top 5 bimodeL Configurations (VFS Score)",
        fontsize=13,
        fontweight="bold",
        pad=15,
    )
    ...
    ax4.set_ylim(0.5, 0.85)  # Focus on the superscalar VFS range
    ...
    plot_path = out_dir / "bimodeL_exploration.png"
    plt.savefig(plot_path, dpi=150)
    print(f"Saved exploration plot: {plot_path}")
    plt.close()

    # Print top 5
    print("\nTop 5 bimodeL Configurations:")
