...
def main():
    ...
    required = ["gag<12,2>", "gap<6,6,2>", "pag<6,8,2>", "pap<2,8,8,2>", "bimode<10,10,8,2>", "lxor<10,4>"]
    missing = [r for r in required if r not in results]
    if missing:
        print(f"Warning: Missing sweep data for {missing}. Using hardcoded fallbacks.")
        fallback = {
            "gag<12,2>": {"name": "GAG", "expr": "gag<12,2>", "ipc": 0.9866, "mpi": 0.8294, "epi": 132.0, "vfs": 0.2297},
            "gap<6,6,2>": {"name": "GAP", "expr": "gap<6,6,2>", "ipc": 0.9874, "mpi": 0.7748, "epi": 131.0, "vfs": 0.2309},
            "pag<6,8,2>": {"name": "PAG", "expr": "pag<6,8,2>", "ipc": 0.9645, "mpi": 0.9665, "epi": 72.0, "vfs": 0.2233},
            "pap<2,8,8,2>": {"name": "PAP", "expr": "pap<2,8,8,2>", "ipc": 0.9763, "mpi": 0.8542, "epi": 90.0, "vfs": 0.2276},
            "bimode<10,10,8,2>": {"name": "Bi-Mode", "expr": "bimode<10,10,8,2>", "ipc": 0.9803, "mpi": 0.7042, "epi": 127.0, "vfs": 0.2308},
            "lxor<10,4>": {"name": "LXOR", "expr": "lxor<10,4>", "ipc": 0.9717, "mpi": 0.9312, "epi": 161.0, "vfs": 0.2245}
        }
        for m in missing:
    ...
    comparison_list = [
        ...
        {"name": "Reference\n(CBP2025)", "expr": "reference", "ipc": 0.987, "mpi": 0.491, "epi": 0.0, "vfs": 0.0, "is_baseline": True},
        # Our Best Predictors
        {**results["gag<12,2>"], "is_baseline": False, "name": f"GAG\n{results['gag<12,2>']['expr']}"},
        {**results["gap<6,6,2>"], "is_baseline": False, "name": f"GAP\n{results['gap<6,6,2>']['expr']}"},
        {**results["pag<6,8,2>"], "is_baseline": False, "name": f"PAG\n{results['pag<6,8,2>']['expr']}"},
        {**results["pap<2,8,8,2>"], "is_baseline": False, "name": f"PAP\n{results['pap<2,8,8,2>']['expr']}"},
        {**results["bimode<10,10,8,2>"], "is_baseline": False, "name": f"Bi-Mode\n{results['bimode<10,10,8,2>']['expr']}"},
        {**results["lxor<10,4>"], "is_baseline": False, "name": f"LXOR\n{results['lxor<10,4>']['expr']}"}
    ]
    ...
    colors = ['#7f7f7f', '#4A90E2', '#50E3C2', '#F5A623', '#E284B3', '#A890E2', '#7ED321']

    # Create 2x2 grid of plots
    fig, axs = plt.subplots(2, 2, figsize=(16, 12))
    fig.suptitle("Best-of-Class Head-to-Head Comparison", fontsize=18, fontweight='bold', y=0.98)
