#!/usr/bin/env python3
import sys
import argparse
import csv
from pathlib import Path

PROFILING_DIR = Path(__file__).resolve().parent
REPO_ROOT = PROFILING_DIR.parent
sys.path.append(str(PROFILING_DIR / "lib"))

from profiling_core import run_predictor_on_multiple_traces, get_predictor_size_bits

CONFIGS = {
    "8KB": [
        {"name": "tage_simple", "expr": "tage_simple<10,16,64,4,16,64,3>"},
        {"name": "tage_simple_u", "expr": "tage_simple_u<10,15,64,4,16,64,3>"},
        {"name": "tage_simpleL", "expr": "tage_simpleL<8,12,64,4,16,64,3,4>"},
        {"name": "tage_simple_uL", "expr": "tage_simple_uL<8,12,64,4,16,64,3,4>"}
    ],
    "16KB": [
        {"name": "tage_simple", "expr": "tage_simple<11,16,64,4,16,64,3>"},
        {"name": "tage_simple_u", "expr": "tage_simple_u<11,15,64,4,16,64,3>"},
        {"name": "tage_simpleL", "expr": "tage_simpleL<9,16,64,4,16,64,3,4>"},
        {"name": "tage_simple_uL", "expr": "tage_simple_uL<9,15,64,4,16,64,3,4>"}
    ]
}

def main():
    parser = argparse.ArgumentParser(description="Compare TAGE predictor variants under fixed budget")
    parser.add_argument("--tracedir", default=str(REPO_ROOT / "traces"), help="Directory containing traces")
    parser.add_argument("--outdir", default=str(PROFILING_DIR / "outputs"), help="Output directory")
    parser.add_argument("--warmup", type=int, default=1000000)
    parser.add_argument("--measure", type=int, default=10000000) # 10M is a good compromise for speed
    parser.add_argument("--jobs", type=int, default=8, help="Parallel simulation jobs")
    parser.add_argument("--skip-sim", action="store_true", help="Skip simulation and use existing results")
    args = parser.parse_args()

    tracedir = Path(args.tracedir).resolve()
    outdir = Path(args.outdir).resolve()
    outdir.mkdir(parents=True, exist_ok=True)
    csv_path = outdir / "tage_comparison_results.csv"

    # Select traces: try to load selected_extreme_traces.txt
    extremes_file = outdir / "selected_extreme_traces.txt"
    traces = []
    if extremes_file.exists():
        with open(extremes_file, "r") as f:
            extreme_names = [line.strip() for line in f if line.strip()]
        for name in extreme_names:
            # Look for gzip file
            gz_path = tracedir / f"{name}_trace.gz"
            if gz_path.exists():
                traces.append(gz_path)
    
    if not traces:
        # Fallback to scanning tracedir
        traces = sorted(list(tracedir.glob("*_trace.gz")))

    if not traces:
        # Final fallback to test trace
        test_trace = REPO_ROOT / "gcc_test_trace.gz"
        if test_trace.exists():
            traces = [test_trace]
        else:
            print("Error: No traces found for simulation.")
            sys.exit(1)

    print(f"Running TAGE comparison on traces: {[t.name for t in traces]}")
    results = []

    if not args.skip_sim:
        for budget in ["8KB", "16KB"]:
            print(f"\n--- TAGE {budget} Comparisons ---")
            for cfg in CONFIGS[budget]:
                expr = cfg["expr"]
                size_bits = get_predictor_size_bits(expr)
                print(f"Running {cfg['name']} ({expr}) [Size: {size_bits} bits]...")
                try:
                    run_results = run_predictor_on_multiple_traces(expr, traces, args.warmup, args.measure, args.jobs)
                    for trace_name, metrics in run_results.items():
                        results.append({
                            "budget": budget,
                            "name": cfg["name"],
                            "expr": expr,
                            "size_bits": size_bits,
                            "trace": trace_name,
                            **metrics
                        })
                except Exception as e:
                    print(f"Failed running {expr}: {e}")

        # Save results to CSV
        with open(csv_path, "w", newline="") as f:
            if results:
                writer = csv.DictWriter(f, fieldnames=results[0].keys())
                writer.writeheader()
                writer.writerows(results)
        print(f"\nResults saved to {csv_path}")
    else:
        if not csv_path.exists():
            print(f"Error: CSV file not found: {csv_path}")
            sys.exit(1)
        with open(csv_path, "r") as f:
            reader = csv.DictReader(f)
            results = [dict(row) for row in reader]
        print(f"\nLoaded cached results from {csv_path}")

    # Print summary table
    print("\n" + "="*80)
    print(f"{'Budget':<8} | {'Predictor Name':<16} | {'Trace':<16} | {'MPKI':<8} | {'IPC':<8} | {'VFS Score':<10}")
    print("="*80)
    # Sort results for presentation
    results.sort(key=lambda x: (x["budget"], x["name"], x["trace"]))
    for r in results:
        print(f"{r['budget']:<8} | {r['name']:<16} | {r['trace']:<16} | {float(r['mpki']):.4f} | {float(r['ipc']):.4f} | {float(r['vfs']):.6f}")
    print("="*80)

if __name__ == "__main__":
    main()
