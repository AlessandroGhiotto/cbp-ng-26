...
def main():
    ...
    if not args.skip_sim:
        ...
        for B in [6, 8, 10, 12]:
            # gagL (PC_B = 0)
            configs.append({"type": "gagL", "expr": f"gagL<{B},2,4>", "B": B, "PC_B": 0, "BHR_B": B})
            # gapL (PC_B = 2, 4, 6, ...)
            for pc in range(2, B, 2):
                bhr = B - pc
                configs.append({"type": "gapL", "expr": f"gapL<{bhr},{pc},2,4>", "B": B, "PC_B": pc, "BHR_B": bhr})

        print(f"Sweeping {len(configs)} gagL vs gapL configurations...")
        for cfg in configs:
            expr = cfg["expr"]
            print(f"Running: {expr} (Total bits: {cfg['B']}, PC bits: {cfg['PC_B']}, History bits: {cfg['BHR_B']})")
            try:
        ...
        with open(csv_path, "w", newline="") as f:
            fieldnames = ["type", "expr", "B", "PC_B", "BHR_B", "ipc", "cpi", "epi", "mpki", "vfs"]
    ...
    fig.suptitle("gagL vs gapL: Superscalar Indexing Space Sweep", fontsize=18, fontweight='bold', y=0.98)
    ...
    ax1.set_title("Accuracy vs Index Size", fontsize=13, fontweight='bold', pad=15)
    ax1.set_xlabel("Total Index Bits (BHR_B + PC_B)", fontsize=11, fontweight='bold')
    ...
    ax2.set_title("Performance (IPC) vs Index Size", fontsize=13, fontweight='bold', pad=15)
    ax2.set_xlabel("Total Index Bits (BHR_B + PC_B)", fontsize=11, fontweight='bold')
    ...
    ax3.set_title("Energy vs Index Size", fontsize=13, fontweight='bold', pad=15)
    ax3.set_xlabel("Total Index Bits (BHR_B + PC_B)", fontsize=11, fontweight='bold')
