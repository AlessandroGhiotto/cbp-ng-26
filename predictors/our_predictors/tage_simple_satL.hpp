 ...
 * If the bimodal base predictor has a fully saturated counter, TAGE lookup is skipped.
 */
...
template <
    ...
    >
struct tage_simple_satL : predictor
{
    ...
    val<1> predict2(val<64> inst_pc) override
    {
        ...
        // Check if bimodal counter is saturated for this offset
        arr<val<1>, LI> sat_arr = reg_saturated.make_array(val<1> {});
        val<1> skip_tage = sat_arr.select(offset);
