...
template <u64 PC_B = 8, u64 BHR_B = 8, u64 COMPLEMENT_B = 4, u64 CTR_B = 2, u64 LINE_B = 4>
struct lxor_generalL : predictor
{
    /*
     ...
     * to index a line in a single pattern history table of 2-bit saturating counters.
     *
     * By predicting a line of counters which is long 2^LINE_B, we predict 2^LINE_B
     * instructions per cc.
     */
    ...
    val<1> predict1([[maybe_unused]] val<64> inst_pc)
    {
        ...
        block_pc = val<PC_B> { inst_pc >> (2 + LINE_B) };
        bhr = bhrs.read(block_pc);
...
template <u64 PC_B = 8, u64 BHR_B = 4, u64 LINE_B = 4>
struct lxorL : lxor_generalL<PC_B, BHR_B, BHR_B, 2, LINE_B>
{
    // Inherit the constructor from the parent class
    using lxor_generalL<PC_B, BHR_B, BHR_B, 2, LINE_B>::lxor_generalL;
};
